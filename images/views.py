...
from .serializers import (
    ...
    BasicImageDetailOutputSerializer,
)
...
class ImagesView(generics.ListCreateAPIView):
    ...
    def get_serializer_class(
        self,
    ) -> Type[ImageOutputSerializer] | Type[BasicImageOutputSerializer]:
        user_profile = self.request.user.userprofile
        account_tier = user_profile.account_tier

        if account_tier and (
            account_tier.name in ["Premium", "Enterprise"]
            or account_tier.original_image_link
        ):
...
class ImageDetailView(views.APIView):
    ...
    def post(self, request: Request, image_id: int) -> Response:
        ...
        if is_enterprise or has_expiring_link:
            ...
            return Response(
                {"temporary_link": temporary_link}, status=status.HTTP_201_CREATED
            )
